...
void Game::_Update(float dt)
{
             ...
             _pBall->SetVelocity(Vector2f(60.0f, 0.0f));
            
            // Chequea si el objeto ha salido de la ventana
            if (_pBall->GetPosition().x > 800.0f)
            {
                //el objeto vuelve a la posición inicial
                _pBall->SetPosition(Vector2f(0.0f, 400.0f));
            
                
            }
            //Aplicamos acceleración de la velocidad al objeto
            if (_pBall->GetVelocity().x == 60.0f || _pBall->GetVelocity().x < 120)  {
               
                _pBall->AddAcceleration(Vector2f(+10.0f, 0.0f));
            }
